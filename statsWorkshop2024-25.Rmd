...
Heatmaps are graphical representations of data, which use a color-code to represent their different values. Colour variation is analogous to the intensity of the data values,a nd they are used to visually inform the reader about a phenomenon. 
<br> z-standarization is commonly used in heatmap generation.

<br> What is data z-standarization?
<br> It is a statistical procedure used to make data points from different datasets/samples comparable. In this procedure, each data point is converted into a z-score. A z-score indicates how many standard deviations a data point is from the mean of the dataset.

<br> How do we calculate the z-score?

```{r, echo=FALSE, out.width="50%", fig.cap="z-score calculation formula."}

knitr::include_graphics("images/image-z-score_formula-cropped.jpg")
```
z-value is the value we want to calculate.
<br> x is the observed value.
<br> μ is the mean value of the samples (per row).
<br> σ is the standard deviation of the sample.
<br> Be cautious, as μ and σ are actually the mean and the standard deviation of the population, but in our case we have a limited number of samples. Please search online how somebody can estimate , under certain conditions, the mean and the standard deviation using a sample.

<br> Here, you can see 2 ways to vizualize transcriptomic data, using standarization. Initially, we use the heatmap function from base R.
<br> Additionally, we use the pheatmap package, which is commonly used between researchers when they publish their work. 
<br> A 3rd package, which is used for more complicated heatmaps is the following : ComplexHeatmap
...
### 3.5. Other graph types and their interpretation
#### A. Volcano plots
Volcano plots are commonly used graphs to show the results of omics experiments. They are used regularly in RNAseq experiments to diplay Differentially Expressed Genes (DEGs), as somebody can easily identify biologically significantly changing genes.
<br> They are scatterplots that show statistical significance (p-val) vs magnitude of change (FC - Fold Change). Usually, The up-regulated genes are towards the right, while the down-regulated towards the left.

```{r, collapse= TRUE, include = TRUE, echo = FALSE}
ggdraw() + 
  draw_image("images/image-volcano1.png", width = 0.33) + 
  draw_image("images/image-VocanoPlot.svg", width = 0.33, x = 0.35) +
  draw_image("images/image-VolcanoPlot_2.svg", width = 0.33, x = 0.7)
```
<br>
...
```{r, collapse= TRUE, include = TRUE, echo = FALSE}
ggdraw() + 
  draw_image("images/image-manhattan.jpg", width = 1) 
```
<br>
...
```{r, collapse= TRUE, include = TRUE, echo = FALSE}
ggdraw() + 
  draw_image("images/image-vennDiagram.png", width = 1) 
```
...
### 3.6. Choose the right plot for your data
<br>
...
```{r, collapse= TRUE, include = TRUE, echo = FALSE}
ggdraw() + 
  draw_image("images/image-chartType.png", width = 1) 
```
...
library(cowplot)
ggdraw() + 
  draw_image("images/image-1800502499.png", width = 0.5) + 
  draw_image("images/image-1781426681.png", width = 0.5, x = 0.5, y = -0.05)
...
<br> R's t.test() function perform's a Welch's t-test by default. A Welch's t-test does not assume equal variance. It should be used if there is any doubt about equal variance.
<br> In R to perform a two-sample t-test, we need to add the option var.equal =TRUE
...
<br>
Theory behind using t_test() function
<br>
One-sample test
<br>
:::::::::::::::::::::::::::::::::::::::::
<br>
df %>% t_test(df$columnx ~ 1, mu = 0)
<br>
...
:::::::::::::::::::::::::::::::::::::::::
<br>
df %>% t_test(df$columnx ~ df$columny)
<br>
...
:::::::::::::::::::::::::::::::::::::::::
<br>
df %>% t_test(df$columnx ~ df$columny, paired = TRUE)
<br>
...
When our data are not "normal" (normally distributed), instead of a parametric test we could  use a non-parametric alternative.
...
ht = hux(
  ...
  "Non-parametric test" = c("Mann-Whitney U test, also known as Mann-Whitney-Wilcoxon", "Wilcoxon signed-rank test, also known asWilcoxon-Whitney-wilcoxon", "Kruskal-Wallis"),
  add_colnames = TRUE
)
...
### 4.6. Comparing frequencies and Forest plots

Both the Chi-square test and Fisher’s Exact Test are statistical tests used to assess the association or independence between two categorical variables. They are commonly applied to contingency tables, particularly 2x2 tables, to test if the observed distribution of data is significantly different from what would be expected under the null hypothesis (no association between variables).

The Chi-square test is used when the sample size is large, and expected cell frequencies are 5 or more. The Fisher’s Exact Test is used when the sample size is small or when expected frequencies are less than 5.

```{r, collapse= TRUE, include = TRUE, echo = FALSE}
library(ggplot2)
library(cowplot)
combinedPlots <- ggdraw() +
  draw_image("images/image-Forest_Plot_1.png", width = 0.5) + 
  draw_image("images/image-Forest_Plot_2.jpg", width = 0.5, x = 0.5, y = -0.05)
...
# Defining your 2x2 contingency table
table <- matrix(c(15, 212, 25, 223), nrow = 2, dimnames = list(c("Event", "Non-Event"),  c("Intervention", "Control")))

# Calculate Risk Ratio and CI 
result <- epi.2by2(dat = table, method = "cohort.count", conf.level = 0.95)
result

# Executing Chi square test 
chisq.test(table)

# Executing Fisher's Exact Test
fisher.test(table)
...
PCA (Principal Component Analysis) is a dimensionality-reduction method, which is used to reduce the dimensionality of very large datasets.
...
<br>
<br>
You can find more info about clustering at the companion presentation "clusteringMethods.pdf". There we focus on k-means and hierarchical clustering and visually explain it.
<br> It is included in the material we have sent to you.
$$
\\[1cm]
$$
...
k-means clustering is considered one of the simplest clustering algorithms, that groups together certain data with a member of that certain group than with another group.
"k" is the number of groups that we cluster our data points, while "mean" refers to the center of the data in that group.
...
Here, this technique considers all the data points a single cluster, and in each iteration, data points are seperated from the cluster, which are not similar.
This is a procedure that continues till we are left with n number of clusters.

To visualize hierarchical clustering, we use dendrograms, which are tree-like diagrams.
In these, the sequences of merges or splits is recorded.
...
### 5.5. t-SNE and UMAP
Dimension reduction helps to visualize high-dimensional datasets. To find and render visually meaningful patterns in genetic and genomic data, a researcher will likely turn to a class of statistical and computational tools for dimension reduction.
<br>

 t-distributed stochastic neighbor embedding (t-SNE) and uniform manifold approximation and projection (UMAP) are among the tools, in addition to PCA, that life scientists turn to for dimension reduction. While the scope of this workshop is not to teach you how to perform these 2 aforementioned techniques, we want you to be able to visually inspect outcome plots related to these.
 <br>
 
 t-SNE is an algorithm that takes a high-dimensional dataset (such as single-cell RNAseq data) and reduces it to a low-dimensional plot that retains  a lot of the original information. The many dimensions of the original dataset are the thousands of gene expression counts per cell from a scRNA sequencing experiment. This is reduced to a degree that each cell gets a location on a two or three-dimensional plot.
 <br> 
 
 Goal of t-SNE is to place similar cells together and different cells further apart based on their transcriptomic profile. 
 <br>
 
UMAP is an alternative to t-SNE. Here, the UMAP algorithm first learns the original dataset's manifold to calculate cell similarities. Then, it aims to capture these similarities in a low-dimensional plot. What is different is the mathematical approach to build the plot. 
<br>
Broadly speaking, t-SNE is more accurate in vizualizing similar cells, while UMAP more accurately represents the distances between clusters.

Ref: https://www.scdiscoveries.com/blog/knowledge/what-is-t-sne-plot/

```{r, collapse= TRUE, include = TRUE, echo = FALSE}
ggdraw() + 
  draw_image("images/image-tSNE.png", width = 1) 
```
...
library(FactoMineR)
library(factoextra)
exerciseDataset <- subset(penguins, species=="Adelie")
exerciseDataset <- exerciseDataset[,c("body_mass_g", "flipper_length_mm","bill_length_mm")]
exerciseDataset <- na.omit(exerciseDataset)
#install.packages("missMDA")
library(missMDA)
PCA.results2 = PCA(exerciseDataset, graph = T)

# fviz_screeplot(PCA.results2)

# Elbow method
fviz_nbclust(exerciseDataset, kmeans, method = "wss")
# Silhouette
fviz_nbclust(exerciseDataset, kmeans, method = "silhouette")

k2 <- kmeans(exerciseDataset, centers = 2)

# exerciseDataset$cluster = as.character(k2$cluster)

fviz_cluster(k2, data = exerciseDataset)

exerciseDataset$cluster = as.character(k2$cluster)
p1 <- ggplot(aes(x=body_mass_g), data = exerciseDataset) +
  geom_histogram(binwidth = 200) +
  facet_wrap(~cluster) +
  theme_bw()
p1
...
wilcox.test(exerciseDataset$body_mass_g ~ exerciseDataset$cluster)

boxplot(exerciseDataset$body_mass_g ~ exerciseDataset$cluster, col = c("darkblue","darkred"), xlab = "cluster", ylab = "Body mass (g)")
